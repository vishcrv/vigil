...
│   ├── tools/                     # ML-owned except escalate.py
│   │   ├── eda.py
│   │   ├── feature_eng.py
│   │   ├── anomaly.py
│   │   ├── risk.py
│   │   ├── explain.py
│   │   └── escalate.py            # app-owner-owned: writes flags.escalated_at
│   ├── scripts/
│   │   └── enrich.py              # one-time: raw CSV -> enriched Parquet
│   ├── notebooks/
│   │   └── 01_exploration.ipynb   # ML owner's Phase 1 EDA notebook
│   ├── data/
│   │   ├── raw/                   # gitignored: HI-Small_Trans.csv, HI-Small_Patterns.txt, accounts
│   │   └── HI-Small_Enriched.parquet       # gitignored, generated by enrich.py; Agent_Ready.parquet cut, see Database Schema
│   ├── ml/
│   │   └── data.py                # shared lru_cache'd DuckDB connection over the enriched Parquet
│   └── tests/                     # pytest: tools, rule engine, risk classification, intent parsing
└── frontend/
    ├── .env                        # gitignored
    ├── .env.example
    ├── package.json

...
from ml.data import MODEL_DIR as _MODEL_DIR, RULE_HITS_PATH as _RULE_HITS_PATH, get_connection
from ml.feature_eng import build_where, validate_scope
from ml.features import FEATURE_COLUMNS, FEATURE_SQL

# Paths come from ml.data so DATA_DIR is the only knob; see the note there.
MODEL_DIR = Path(_MODEL_DIR)
RULE_HITS_PATH = Path(_RULE_HITS_PATH)
...
TOP_ROWS = 5
# Caps that keep one tool result inside agent.loop.MAX_TOOL_RESULT_CHARS (20,000). Over that
# the loop swaps the payload for a truncation notice and the chain to risk() breaks.
MAX_RULE_HITS = 25
...
def anomaly(scope: dict, method: str = "all") -> dict:
    ...
    try:
        ...
        return {
            ...
            "rule_hits": rule_hits,
            "rule_hits_returned": len(rule_hits),
            "rule_hits_total": rule_hits_total,
            "rule_hits_truncated": rule_hits_total > len(rule_hits),

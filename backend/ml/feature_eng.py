...
MAX_LIMIT = 1000
# Default kept small because this crosses the agent loop's context budget: 200 records
# serialised to ~92,000 chars against agent.loop.MAX_TOOL_RESULT_CHARS of 20,000, and an
# over-budget result is replaced wholesale by a truncation notice. `aggregate` still
# describes the full scope, and a caller that wants more rows can raise `limit`.
DEFAULT_LIMIT = 25
